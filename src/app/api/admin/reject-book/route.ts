...
export async function GET(request: NextRequest) {
  ...
  if (!verifyApprovalToken(bookId, token)) {
    return html(
      `<h1>🔒 Invalid Token</h1><p>This rejection link is invalid or has been tampered with.</p>`,
      "Invalid Token"
    );
  }
  ...
  console.log(`[admin] Book ${bookId} (${book.child_name}) REJECTED`);

  return html(
    `<h1>🚫 Book Rejected</h1>
     <p><strong>${book.child_name}'s</strong> book has been marked as rejected.</p>
     <p style="margin-top:12px">Please contact the customer directly to let them know and arrange any refund if applicable.</p>`,
    "Book Rejected"
  );
}
